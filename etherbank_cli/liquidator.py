...
@main.command()
@click.option('--liquidation-id', type=int, required=True, help="The liquidation's ID")
...
def place_bid(liquidation_id, ether, private_key):
    ...
    func = utils.liquidator_contract.functions.placeBid(liquidation_id, int(ether*10**18))
...
def stop_liquidation(liquidation_id, private_key):
    ...
    func = utils.liquidator_contract.functions.stopLiquidation(liquidation_id)
    tx_hash = utils.send_transaction(func, 0, private_key)
    return tx_hash
...
def get_best_bid(liquidation_id, private_key):
    ...
    func = utils.liquidator_contract.functions.getBestBid(liquidation_id)
...
def withdraw(private_key):
    ...
    func = utils.liquidator_contract.functions.withdraw()
    tx_hash = utils.send_transaction(func, 0, private_key)
    return tx_hash
...
def _active_liquidations(liquidation_id=None):
    ...
    start_filter = utils.liquidator_contract.events.StartLiquidation.createFilter(
    ...
    for liquidation_bytes in liquidations:
        ...
        stop_filter = utils.liquidator_contract.events.StopLiquidation.createFilter(
            fromBlock=1,
            toBlock='latest',
    ...
    for liquidation in sorted(result, key=lambda liquidation: liquidation['liquidationId']):
        click.secho('liquidationId:\t{}'.format(liquidation['liquidationId']), fg='green')
        click.secho('loanId:\t\t{}'.format(liquidation['loanId']), fg='green')
        click.secho('collateral:\t{} ether'.format(liquidation['collateralAmount']*10**-18), fg='green')
        click.secho('loan:\t\t{} dollar'.format(liquidation['loanAmount']*10**-2), fg='green')
        click.secho('startTime:\t{}'.format(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(liquidation['startTime']))), fg='green')
        click.secho('endTime:\t{}'.format(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(liquidation['endTime']))), fg='green')
        click.secho()
    if not result:
